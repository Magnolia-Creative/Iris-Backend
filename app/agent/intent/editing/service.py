...
from app.agent.intent.editing.compiler import IntentCompiler
...
class IntentLLMCompiler:
    ...
    def _semantic_prompt(self, prompt: str, context: IntentCompilerContext) -> str:
        return (
            ...
            "- Natural trim amounts must become structured amount objects, e.g. '1 second', 'two seconds', "
            "'500ms' as type duration with numeric value and unit; 'the first second' or 'first second' "
            "means one second from the clip start (edge start, one second).\n"
